# Consensus Workflow Specification

## Purpose

Defines the consensus loop in which each artifact is reviewed by the configured agents — independently per revision, with capped rounds, blocking, and human checkpoints — before the workflow advances to the next phase.
## Requirements
