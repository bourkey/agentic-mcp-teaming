...
**Alternative considered**: ship a `@bourkey/peer-bus-client` npm package. Rejected — doubles the release pipeline, forces version-skew management between client and server, and offers no real benefit for a codebase this small.
...
3. The consumer-repo companion proposal gets rewritten against the new client CLI surface (much smaller than its prior draft).
...
- "extensible, not just for consumer-repo" → D1–D3 and D9
