...
- **Option B** — require `body = { event: string, …rest }` with an initial enum of event names (`worktree-created`, `phase-change`, `ship-requested`, `merge-complete`) that the companion consumer-repo change will emit. Stronger contract; forces both changes to evolve together.
