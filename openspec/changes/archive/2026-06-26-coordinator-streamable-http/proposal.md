...
- **Client-side migration**: no forced migration. `consumer-repo/.mcp.json` and any other `type: "http"` client moves its URL from `…/sse` to `…/mcp` and keeps `type: "http"`; `type: "sse"` clients are untouched.
