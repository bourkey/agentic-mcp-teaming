...
- `templates/opsx/commands/` — one markdown per opsx command (11 files):
  ...
  - `draft.md`, `merge.md`, `review.md`, `ship.md`, `status.md`, `sync.md`, `worktree.md` — adapted from consumer-repo by collapsing the area topology. Every `consumer-repo-<area>` path becomes `<repo-basename>-feature`; every area-rotation branch becomes a single-slot branch; the paused-branch mechanism survives as single-depth (one pause at a time) rather than cross-area stacked.
...
Task 7.1 applies the scaffold to `agentic-mcp-teaming` — writes the 11 commands, updates the existing 4, writes `.opsx-toolkit/` files. From then on, changes in this repo use the same workflow as consumer-repo (just single-slot instead of area).
...
- **Bidirectional portability**: projects using the scaffolded toolkit benefit from future improvements via `opsx-upgrade`; projects that already have handcrafted commands (consumer-repo) are not affected until they explicitly scaffold.
...
- **Explicitly deferred** (each its own follow-up):
  - Area topology (`--topology area`) — the richer variant consumer-repo uses. Port once we have a second consumer actually needing it.
  ...
  - Custom reviewer personas (the `.claude/agents/` files consumer-repo ships). They're review-gate implementation details, not part of the opsx CLI.
