...
The opsx workflow started as consumer-repo's internal tooling, evolved through many iterations (priority inversion, paused-branch mechanism, cross-area merge rotation, review gates), and has stabilised into a shape that's worth porting to other projects. But the commands are tightly coupled to consumer-repo's specifics: 3-area topology (frontend/backend/misc), `../consumer-repo-<area>` worktree paths, and a merge queue sorted by area+priority.
...
**Non-Goals:**
- Area topology variant (deferred to a follow-up when demand exists beyond consumer-repo).
...
- Modification of consumer-repo's handcrafted commands. It stays as-is until it chooses to migrate.
...
Every command template originates from a consumer-repo source:
...
| `draft.md` | consumer-repo | replace sibling-path scan with single-slot scan at `$FEATURE_PATH/openspec/drafts/` |
| `merge.md` | consumer-repo | drop area rotation logic; collapse 3-slot loop to single rebase-of-the-feature-worktree |
| `review.md` | consumer-repo | replace `cd ../consumer-repo-<area>` error message with `cd $FEATURE_PATH` |
| `ship.md` | consumer-repo | drop area resolution from archive lookup; otherwise unchanged |
| `status.md` | consumer-repo | collapse 3-row table to 1-row; drop area column |
| `sync.md` | consumer-repo | collapse 3-slot worktree identification to single slot |
| `worktree.md` | consumer-repo | collapse 3-area selection to single slot; simplify paused-branch to one-depth |

Each commit of a template file in `templates/opsx/commands/` is a transparent adaptation — no reinvention. Reviewers can diff against the consumer-repo source to see what survived.
...
- **Risk**: the adapted commands have bugs not present in the consumer-repo originals (introduced during area → single-slot collapse).
