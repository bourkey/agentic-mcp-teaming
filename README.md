...
- **DNS-rebinding** — set `allowedHosts` to the exact `Host` values clients send (e.g. `coordinator.example.dev:3100`); `/mcp` requests with any other `Host` get a `403`. It is **not** derived from `host` (your bind address is not the client-visible hostname). Omit `allowedHosts` to leave the check off.
