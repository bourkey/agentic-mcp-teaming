...
### TLS and cross-machine deployment

By default the coordinator listens over **plain HTTP on `127.0.0.1`** — fine for a single machine. To host the coordinator and connect agents from **other machines**, enable TLS so the auth token, `X-Pane-Token`, and message bodies are encrypted in transit. TLS is opt-in via a `tls` block in `mcp-config.json`:
...
```json
{
  ...
  "allowedHosts": ["coordinator.example.dev:3100"]
}
```
...
- **DNS-rebinding** — set `allowedHosts` to the hostnames clients use (e.g. `coordinator.example.dev`); requests with any other `Host` get a `403` on **both** `/mcp` and the `/sse` stream open. Each bare host is auto-expanded to also accept `host:port`, so a client sending the port still matches. It is **not** derived from `host` (your bind address is not the client-visible hostname). Omit `allowedHosts` to leave the check off.
- **advertisedHost** — when `host` is a wildcard (`0.0.0.0`/`::`), set `advertisedHost` to a routable address so sub-agents spawned via `invoke_agent` get a reachable `COORDINATOR_MCP_URL` callback (a wildcard is not a valid destination). Defaults to `host`.
- There is intentionally **no option to disable TLS verification.** For a self-signed/private CA, trust the CA on the client; do not look for an insecure flag.

A TLS-terminating reverse proxy (Caddy/nginx, with ACME-managed certs) in front of the loopback coordinator is a supported alternative — see the runbook.
...
```bash
./scripts/launchd/install.sh     # writes ~/Library/LaunchAgents/dev.bourkey.agentic-mcp-coordinator.plist and loads it
```
