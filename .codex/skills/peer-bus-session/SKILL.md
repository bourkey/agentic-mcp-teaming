...
description: Registers the Codex pane with the coordinator bus and drains the mailbox each turn. Activates automatically when `$COORDINATOR_SESSION_NAME` is set by the tmux launcher.
license: MIT
metadata:
  version: "1.0"
---

This skill attaches the current Codex pane to the peer-session-bus
coordinator running at the MCP server registered as `coordinator` (see
`.mcp.json -> mcpServers.coordinator`). It is auto-activated by your tmux launcher, which sets
`$COORDINATOR_SESSION_NAME` in each worktree window to a Codex-scoped
name (`codex-main`, `codex-frontend`, `codex-backend`, `codex-misc`).
...
2. **Identity validation.** Read `$COORDINATOR_SESSION_NAME`. If unset,
   STOP silently (the pane was started outside the launcher — bus features
   are not expected here). If set to any value OTHER than `codex-main`,
   `codex-frontend`, `codex-backend`, or `codex-misc`, STOP and print once:
   `peer-bus: COORDINATOR_SESSION_NAME='<value>' is not a recognised pane; bus features disabled`.
3. **Register.** Call the MCP tool
   `register_session({ name: $COORDINATOR_SESSION_NAME })` against
   `mcpServers.coordinator`. The returned `sessionToken` is a 256-bit random
   base64url string. The X-Pane-Token header is sent automatically by the MCP transport (see `.mcp.json` → `mcpServers.coordinator.headers`).
...
5. **Per-turn startup degradation.** If the MCP tool `register_session` is
   ...
   (rate-limited to once per Codex session per section 5's taxonomy)
   ...
   from step 3 — a coordinator that came up after Codex started will
...
- **`invalid_session_token`** — the coordinator forgot us or restarted.
  Attempt recovery **at most once per Codex turn**:
  1. Call `register_session({ name: $COORDINATOR_SESSION_NAME })`.
...
- **Token missing from working context at turn start** — if the skill's
  ...
  alive), treat this as an uninitialized state and call
  `register_session({ name: $COORDINATOR_SESSION_NAME })`
...
- **Transport-level failure** (connection reset, SSE stream closed,
  ...
  logged at most once per Codex session (see section 5).
...
- **Transport-level failure.** If `read_messages` fails with a transport-
  ...
  inbox summary. At most one informational log per Codex session.
...
- **Origin allowlist.** Validate the envelope's `from` attribute against
  the allowlist `{codex-main, codex-frontend, codex-backend, codex-misc}` (regardless of `kind`)
...
- **Secret redaction.** Drop (do NOT summarise, do NOT render verbatim)
  ...
  - A generic high-entropy string `[A-Za-z0-9+/_=-]{40,}` UNLESS the field
    name is in the shape-allowlist `{baseSha, commitSha, messageId}`
...
**Rate limit.** Each error class logs at most ONCE per Codex session.
